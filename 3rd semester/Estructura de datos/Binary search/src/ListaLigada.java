public class ListaLigada<T extends Comparable<T>> {
    ...
    private int preBinarySearch(T elemento, int inicio, int ultimo) {
        while (inicio<=ultimo) {
            int mid = (inicio+ultimo)/2;
            if (elemento.compareTo(encontrarElementoEnElIndice(mid)) == 0) {
                return mid;
            }else{
                if (elemento.compareTo(encontrarElementoEnElIndice(mid)) < 0) {
                    ultimo = mid-1;
    ...
    public int binarySearch(T elemento) {
        return preBinarySearch(elemento, 0, contarElementos()-1);
    }
}